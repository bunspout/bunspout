...
Fast Excel Read/Write Library for Bun and Node.js.

A fast, streaming Excel read/write library for XLSX files. Built with Bun runtime for high-performance data processing, with runtime-agnostic test infrastructure supporting both Bun and Node.js.
...
bun test src/xlsx/*.test.ts # Run specific tests
```

### Local Testing (Node.js)
```bash
npm run test:node           # Run all tests in Node.js (needs to be available and npm installed)
```
...
npm run docker:test:bun
npm run docker:test:node

# Interactive shell
npm run docker:shell:bun
npm run docker:shell:node
```

## Runtime-Agnostic Tests

Tests use a runtime-agnostic framework that works in both Bun and Node.js:

```typescript
// Use @tests/framework instead of bun:test
